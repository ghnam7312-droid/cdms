...
// 양식 N68(프로젝트 품의서). contents 가 본문을 대체하므로 N68 표 서식을 본문으로 구성한다.
...
function n68Body(prog: any): string {
  ...
  const B = "border:1px solid #888;padding:5px";
  const H = B + ";background:#f2f2f2;font-weight:bold;text-align:center";
  const lab = (l: string, v: string) =>
    `<tr><td style="${B};background:#f2f2f2;font-weight:bold;width:120px">${l}</td><td colspan="7" style="${B}">${v}</td></tr>`;
  const blanks = (n: number) => { let s = ""; for (let i = 0; i < n; i++) s += `<td style="${B}"></td>`; return s; };
  const labRow = (l: string) => `<tr><td style="${B};background:#f2f2f2;font-weight:bold">${l}</td>${blanks(7)}</tr>`;
  // 외주경비·내부비용 입력행: 높이를 키워 작성 공간 확보
  const tallCells = (n: number) => { let s = ""; for (let i = 0; i < n; i++) s += `<td style="${B};height:40px"></td>`; return s; };
  const blankRow = () => `<tr style="height:40px">${tallCells(8)}</tr>`;

  let o = `<table style="border-collapse:collapse;width:100%;font-size:13px">`;
  o += lab("프로젝트명", "");
  o += lab("발주처", esc(prog.client));
  o += lab("주요 내용", "<br><br>");
  o += lab("계약기간", esc(period));
  o += lab("매출액(VAT제외)", "");
  o += lab("개발방식", "");
  o += `<tr><td style="${H}">외주경비</td><td style="${H}">구분</td><td style="${H}">단위</td><td colspan="2" style="${H}">단가</td><td colspan="2" style="${H}">금액</td><td style="${H}">비고</td></tr>`;
  for (let i = 0; i < 11; i++) o += blankRow();
  o += labRow("출장비");
  o += labRow("소계 1");
  o += `<tr><td style="${H}">내부비용</td><td style="${H}">구분</td><td style="${H}">기간</td><td style="${H}">단가</td><td colspan="2" style="${H}">참여율</td><td style="${H}">금액</td><td style="${H}">비고</td></tr>`;
  for (let i = 0; i < 6; i++) o += blankRow();
  o += labRow("소계2");
  o += labRow("잡비용");
  o += labRow("포팅");
  o += labRow("비용 총계");
