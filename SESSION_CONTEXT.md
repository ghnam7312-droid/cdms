...
- **워커 차시 매칭 통일(07-17)**: 워커 match_lesson이 "N차시"를 우선해 차시형 과정의 "01주차 2차시.mp4"(=1차시 2파트)를 2차시로 오인 → 품질 점검이 파트 1개만 인식(데이터사이언스 1차시). Edge와 동일 규칙 추가: 차시형(주차 없음) 과정은 파일명 "N주차"=차시 번호·"N차시"=파트. 주차형은 기존 조합 매칭 유지. 스텁 검증. **워커 재배포 필요**.
- **품질 점검 속도 개선(07-17)**: 원격(NAS2) 점검은 영상 전체 다운로드(파일당 1.5~2.5GB)라 다중 파트 점검이 매우 느림 — ① **증분 점검**: audio_checks에 file_mtime·qc_ver 저장, 파일 변경 없고 기준 버전(QC_VER 상수) 같으면 재분석 생략(재점검 몇 초). 기준 바꾸면 QC_VER 올려야 전체 재분석. ② nas_tasks 처리 시작 시 status='running' 표시 + 3시간 지난 running은 재시도(중단 복구). timedelta import 추가. **워커 재배포 필요**(기준 완화와 함께).
- **품질 점검 기준 완화(07-17)**: 오탐 감소 — AUDIO_JUMP_LU 9→**15**, CCA_MIN_H 14→**20px**, OCR 신뢰도 하한 60→**75**(신규 env CCA_MIN_CONF). 워커 코드 기본값 변경 — **재배포 필요**. 기존 결과는 다음 점검 실행 시 새 기준으로 대체됨. ⚠ 품질점검_기준.docx 문서는 구 기준(9LU·14px) 상태 — 갱신 필요.
- **NAS 자동 매칭 주기 강화(07-17)**: cdms-nas-scan cron을 `*/10 0-14,21-23`(새벽 제외)에서 **`*/4 * * * *`(24시간 4분 간격)**으로 변경 — 실행당 ~110초 예산에 약 20과정 처리(전체 52과정), 과정당 실효 갱신 주기 약 8~12분(≒10분 목표). 동시 실행은 scan_all_lock(150초)으로 방지.
- **품질 점검 후속 수정(07-17)**: ① 파일별 저장을 막던 구 유니크 인덱스 `audio_checks_lesson_uidx` 발견·제거(07-16e 마이그레이션이 이름을 놓침 — duplicate key 오류 원인). 실패했던 딥러닝 2차시·컴퓨터비전 1차시 점검 재큐잉(SQL insert). ② "종편 폴더를 찾지 못했습니다"(자연어처리 1차시)의 실제 원인: **NAS에 해당 과정 폴더가 없음** — 디지스트 NAS2의 실제 폴더는 07_AI어플리케이션(AI최적화이론과응용)이고 자연어처리 폴더는 미생성. nas_root 재지정 또는 폴더 생성 필요(사용자 판단 대기). 워커 원격 분기에 과정 폴더 없음 명확한 오류 메시지 추가(재배포 시 반영).
- **점검 진행 상태 유지(07-16f, POC)**: 점검 버튼의 '분석 중' 표시가 팝업 재오픈 시 초기화되던 문제 — loadAudioSec이 최근 30분 내 미완료 audio_check 작업(nas_tasks, params->>lesson_id)을 조회해 "⏳ 품질 점검 진행 중…"(disabled) 유지 + watchAudioTask(중복 감시 방지)가 완료 시 자동 갱신·_RVQC 캐시 무효화. 빌드 2026-07-16f (qc-running-state).
- **품질 점검 확장(07-16e, POC)**: ① **차시 내 모든 영상(파트) 검수** — audio_checks가 파일별 1행(unique lesson_id→(lesson_id,file_name), file_path 컬럼 추가), 워커 로컬·원격 분기 모두 전체 파트 분석(_qc_basekey로 사본·수정본은 최신 1개, 이번 대상에 없는 파일의 예전 행 정리). **워커 재배포 필요**. ② 결과 UI 파일별 그룹 표시, 구간 클릭 시 **해당 영상**의 그 위치로 이동(audioSeek가 다중 클립이면 글로벌 오프셋, 단일 재생 중 다른 파일이면 clips에서 직접 로드). ③ **검수 타임라인에 이상 구간 마커**(rvQMarks, #rv-qseg — 빨강=심각/주황=주의(명도대비 포함), 클릭 시 이동, 다중 클립은 오프셋 반영) — openReview·clips 로딩 후 렌더. 노드 스텁 검증. 빌드 2026-07-16e (qc-multi,timeline-marks).
...
- **워커 재배포 경로 확정(07-16)**: 워커 스크립트 실제 위치는 **`/home/agent/nas_worker.py`** (~/nas-worker/ 아님!). 재배포: `SCRIPT=$(systemctl cat cdms-nas-worker | grep -oP '\S+nas_worker\.py' | head -1)` → HASH 커밋 고정 다운로드 → `/tmp/nw.py` py_compile 검사 → `sudo cp /tmp/nw.py "$SCRIPT"` → `sudo systemctl restart cdms-nas-worker`. (07-16 do_duration=False 배포 완료, grep 검증 2건)
- **업로드 시 길이 자동 계산(07-15h)**: 영상(mp4/mov/m4v) 업로드가 백신 검사를 통과하면 프런트가 nas-versions `scan`(해당 과정만)을 자동 호출 — 길이(파트 합산)·종편 상태 즉시 갱신(4초 디바운스). + 워커 scan_progress의 구버전 길이 덮어쓰기(최근 1개 파일) 비활성화(do_duration=False, Edge가 단일 소스) — **워커 재배포 필요**(ai-agent 서버 curl). 빌드 2026-07-15h (auto-duration).
- **영상 길이 재계산 로직 수정(07-15, nas-versions v26)**: ① 길이가 한 번 저장되면 파트 추가돼도 갱신 안 되던 문제 — lessons.duration_files(측정 당시 파트 수) 추가, 파트 수가 달라지면 재계산(duration_files null+길이 있음=수동 입력으로 간주·보존). ② 학교공유/ 사본·(수정)본 중복 합산 문제 — baseKey(리비전·(수정)·최종 표기 제거) 기준 최신 1개만 합산. 전체 lessons duration_files=0으로 재검증 트리거(10분 cron이 순차 재계산). 데이터사이언스 4개 차시 검증 완료(4차시 30:52→57:53, 1·2차시 중복 제거 후 기존값 복원). v26 직접 배포 + 리포 반영.
