...
try{
  ...
  $ip = ([System.Net.Dns]::GetHostAddresses('ai-agent') | Where-Object { $_.AddressFamily -eq 'InterNetwork' } | Select-Object -First 1).IPAddressToString
  $L += "resolved ai-agent -> $ip"
  $host9="agent@$ip"
  $L += "== backup =="
