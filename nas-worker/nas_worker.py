...
AUDIO_SILENCE_MIN = float(os.environ.get("AUDIO_SILENCE_MIN", "3.0"))  # 무음 최소 길이(초) — 3초 이상(2026-07-21)
...
# ── 클립 단위 음량 분석 (v7, 2026-07-21) ──
QC_SEG_GAP_DB  = float(os.environ.get("QC_SEG_GAP_DB", "-50"))   # 클립 경계로 보는 무음 임계(dB)
QC_SEG_GAP_S   = float(os.environ.get("QC_SEG_GAP_S", "0.6"))    # 클립 경계 최소 무음 길이(초)
QC_BAL_LU      = float(os.environ.get("QC_BAL_LU", "6"))         # 클립 간 밸런스 허용 편차(LU)
QC_QUIET_MIN_S = float(os.environ.get("QC_QUIET_MIN_S", "20"))   # 기준보다 낮게 "지속" 판정 최소 길이(초)
# 영상 품질 기준
...
CCA_BG_STD   = float(os.environ.get("CCA_BG_STD", "12"))    # 배경 균일도(밝기 표준편차) 상한 — 슬라이드·자막은 단색 배경, 이미지 속 장식 텍스트 배제

QC_VER = "v7-clipbal"  # 품질 점검 기준 버전 (v6: 단색 배경 위 학습 텍스트만 검사 — 이미지·모션그래픽 속 장식 텍스트 제외)
    ...
    try:
        ...
        for fn in sorted(os.listdir(tmpd)):
            ...
            for ln in (o.stdout or "").splitlines()[1:]:
                ...
                # 배경 균일도: 학습 텍스트(슬라이드·자막)는 단색 배경 — 어두운/밝은 쪽 중 한쪽은 균일해야 함.
                #  양쪽 모두 얼룩덜룩하면 이미지·모션그래픽 속 부수적 텍스트(WCAG 제외 대상)로 보고 건너뜀 (2026-07-18)
                def _std(vs):
                    m = sum(vs) / len(vs)
    ...
    try:
        ...
        if stats.get("max") is not None and stats["max"] >= -0.1:
            issues.append({"type": "clip", "start": 0, "end": round(dur or 0, 1),
                           "detail": "피크 — 최대 볼륨 %.1fdB (0dB 도달, 음 깨짐 위험)" % stats["max"]})
        # 트랙은 있으나 사실상 사운드가 들어있지 않은 파일
        if stats.get("mean") is not None and stats["mean"] <= -60:
            issues.append({"type": "no_audio", "start": 0, "end": round(dur or 0, 1),
                           "detail": "사운드 없음 — 전체 평균 음량 %.0fdB" % stats["mean"]})
        # ── 클립(구간) 단위 음량 분석 (v7, 2026-07-21) ──
        #  영상은 인트로 타이틀·학습내용·강사강의·성우·BGM·효과음 등 클립의 연결 구조.
        #  짧은 무음(QC_SEG_GAP_DB↓ QC_SEG_GAP_S초↑)을 클립 경계로 보고 구간을 나눠,
        #  구간별 평균 라우드니스를 기준(강의 구간 = 재생시간 가중 중앙값)과 비교한다.
        #   · 기준 대비 ±QC_BAL_LU 넘는 클립(5초↑)          → balance(클립 밸런스)
        #   · 기준보다 QC_BAL_LU 낮게 QC_QUIET_MIN_S초↑ 지속 → quiet(강사 목소리보다 낮은 진행)
        ...
        sil = [(i0["start"], i0["end"]) for i0 in issues if i0["type"] == "silence"]
        def in_sil(a, b):
            return any(not (b < s0 or a > e0) for s0, e0 in sil)
        segs = []
        def _flush(buf):
            if not buf:
                return
            t0, t1 = buf[0][0], buf[-1][0]
            vals = [v for _, v in buf if v > -70]
            if t1 - t0 < 1.0 or not vals:
                return
            segs.append((t0, t1, 10 * log10(sum(10 ** (v / 10.0) for v in vals) / len(vals))))
        buf, gap0 = [], None
        for t, v in frames:
            if v <= QC_SEG_GAP_DB:
                if gap0 is None:
                    gap0 = t
                if buf and t - gap0 >= QC_SEG_GAP_S:
                    _flush(buf)
                    buf = []
            else:
                gap0 = None
        ...
        _flush(buf)
        ref = None
        longs = sorted([g for g in segs if g[1] - g[0] >= 3.0], key=lambda x: x[2])
        if longs:
            tot = sum(g[1] - g[0] for g in longs)
            acc = 0.0
            for g in longs:
                acc += g[1] - g[0]
                if acc >= tot / 2:
                    ...
                    break
        if ref is not None:
            stats["ref_lufs"] = round(ref, 1)
            for t0, t1, L in segs:
                d = L - ref
                du = t1 - t0
                if in_sil(t0, t1):
                    continue
                if d <= -QC_BAL_LU and du >= QC_QUIET_MIN_S:
                    issues.append({"type": "quiet", "start": round(t0, 1), "end": round(t1, 1),
                                   "detail": "강의 기준(%.0f LUFS)보다 %.0f LU 낮게 %d초 지속" % (ref, -d, du)})
                elif abs(d) >= QC_BAL_LU and du >= 5.0:
                    issues.append({"type": "balance", "start": round(t0, 1), "end": round(t1, 1),
                                   "detail": "클립 밸런스 — 기준 대비 %+.0f LU (%s)" % (d, "높음" if d > 0 else "낮음")})
        ...
        try:
            ...
            ch = 0
        if ch == 1:
            issues.append({"type": "mono", "start": 0, "end": round(dur or 0, 1),
                           "detail": "모노 출력 — 스테레오(2채널)로 출력 필요"})
        if ch >= 2:
            def ch_sil(cidx):
